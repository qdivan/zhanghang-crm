...
21. `T5-21` 交互语义修正：客户开发公司名点击按状态进入线索详情或客户档案，详情页返回路径与来源页面一致。
   - Status: `DONE`
22. `T5-22` 客户档案支持成单后事项提醒与办结回填，并接入 Dashboard 系统待办。
   - Status: `DONE`
23. `T5-23` 新增 `香港公司模板`，可批量生成年审/商业登记等周期事项提醒。
   - Status: `DONE`
24. `T5-24` 常用资料支持 `内部资料/可公开到官网` 范围标记，并提供公开查询接口。
   - Status: `DONE`
25. `T5-25` 到账核对页：按入账账户展示账户汇总与到账流水，并开放给老板/管理员/部门经理及收费只读授权账号。
   - Status: `DONE`
26. `T5-26` 登录页重做为白底低饱和工具入口，压缩营销化文案。
   - Status: `DONE`
